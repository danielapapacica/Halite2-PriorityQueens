...
public class GameMap {
    ...
    public ArrayList<Entity> objectsBetween(Position start, Position target) {
        ...
        /* vedem daca exista nave personale intre pozitia curenta si target*/
        addEntitiesBetween(entitiesFound, start, target, getMyPlayer().getListShips());
    ...
    /* returneaza Map cu planetele sortate dupa distantele fata de centru
    si fata de de nava curenta
    */
       public Map<Double, Planet> nearbyPlanetsByDistance(final Ship ship) {
        ...
        for (final Planet planet : planets.values()) {
            ...
            entityByDistance.put((ship.getDistanceTo(planet)*
                    planet.getDistanceTo(new Position(getWidth()/2, getHeight()/2))), planet);
        }
